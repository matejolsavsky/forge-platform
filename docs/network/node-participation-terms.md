...
**Owner:** WS-011 · **Version:** 0.2 · **Status:** Draft · **Date:** 2026-09-07
...
The Slovak wording is `podmienky-ucasti-uzla.md` (v0.2); both versions carry the same version number and are
changed together.
...
or the platform hosts it on the node's behalf), **D-2026-19** (a node run by a legal guardian) — `registry/decisions.md`.
...
- A node keeps its profile (`PROFILE.md`) according to the **public node profile standard**. The profile reaches
...
that its profile has drifted from the shape the network expects.

## 4. The node vouches for the truth of its data

The data in the profile — capacity, materials, lead times, prices, contacts, interests — are **statements of the
node**. Forge does not verify them, does not confirm them and does not vouch for them, nor for the fulfilment of
orders the node accepts. This applies equally to a profile hosted by the node and to a profile hosted by the
platform on its behalf. Whoever does business with a node does business with that node, not with Forge.
...
- The node **asks** to be entered (an issue in the platform repository or an e-mail; a request for the platform to
  host the profile is made the same way); the entry is made by the **network operator**.
